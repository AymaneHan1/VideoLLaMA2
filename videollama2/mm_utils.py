...
def disable_flash_attention_2():
    """
    Ensures that Flash Attention 2.0 is disabled globally.
    """
...
def process_video_old(
    ...
    if isinstance(video_path, str):
        if video_path.endswith(".gif"):
            ...
            frame_id_list = frame_sample(
                duration, mode=sample_scheme, local_fps=local_fps
            )
            # limit the max input frames
            if len(frame_id_list) > MAX_FRAMES:
                frame_id_list = np.linspace(0, duration - 1, MAX_FRAMES, dtype=int)
            video_data = [
                frame for index, frame in enumerate(video_gif) if index in frame_id_list
            ]
        else:
    ...
    if aspect_ratio == "pad":
        images = [
            Image.fromarray(f.numpy() if isinstance(f, torch.Tensor) else f)
            for f in video_data
        ]
        images = [
            expand2square(image, tuple(int(x * 255) for x in processor.image_mean))
